...
export function priceOpinion(exams: PriceExam[], orgLabel: string): string {
  ...
  if (high.length > 0)
    parts.push(`${high.length}개가 유사 기관 중간값의 2배 이상입니다. 차이가 가장 큰 항목은 ${top.name}(${top.multiple}배, 상위 ${Math.max(1, Math.min(99, top.percentile))}%)입니다.`);
  else if (mid.length > 0)
    parts.push(`${mid.length}개가 유사 기관 중간값의 1.5배 이상입니다. 차이가 가장 큰 항목은 ${top.name}(${top.multiple}배)입니다.`);
  else
