...
import { ready, get, exams } from "../../../lib/goods";
...
export default async function OG({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!d) return renderPriceOg("JEGAP", "한국소비자원 참가격", null, false);
  const top = exams(d)[0];
  const fact = top ? `${top.name} — 중간값의 ${top.multiple}배 · 상위 ${Math.max(1, Math.min(99, top.percentile))}%` : null;
  const meta = [[d.h.sido, d.h.sigungu].filter(Boolean).join(" "), d.h.kind, "한국소비자원 참가격"].filter(Boolean).join(" · ");
